from finance.constants.model import S3_MODEL_BUCKET_NAME, S3_MODEL_DIR_KEY
from finance.constants.prediction_pipeline_constants.file_config import S3_DATA_BUCKET_NAME, PYSPARK_S3_ROOT
from finance.entity.config_entity import PredictionPipelineConfig
from finance.entity.schema import FinanceDataSchema
from pyspark.sql import DataFrame
from finance.cloud_storage import SimpleStorageService
from finance.exception import FinancialException
import os, sys
from finance.config.spark_manager import spark_session
from finance.logger import logger
from typing import List, Tuple
from finance.ml.estimator import S3FinanceEstimator
    ...
    def read_file(self, file_path: str) -> DataFrame:
        try:
            ...
            if file_path.endswith(".json"):
                ...
                parquet_path = file_path.replace(".json", "_converted.parquet")
                df.write.mode("overwrite").parquet(parquet_path)
                df = spark_session.read.parquet(parquet_path)
            elif file_path.endswith(".parquet"):
                df =  spark_session.read.parquet(file_path)
            else:
                raise FinancialException(f"Unsupported file type: {file_path}", sys)
            return df.limit(100)
